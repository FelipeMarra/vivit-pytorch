...
test_loader = DataLoader(
    KineticsDataset(KINETICS_PATH, 'test', N_FRAMES, test_transforms),
...
print("batch class shape:", classes.shape)
print("batch paths len:", len(paths))

# Tensor[B, V, C, T, H, W], where V is the number of views
B, V, C, T, H, W = videos.shape

videos = videos.view(B*V, C, T, H, W)

# %%
videos = videos[28:32].to(torch.uint8).transpose(1,2) # Get the first one 4 views 
print("Last 4 views shape:", videos.shape)
